...
export class ListComponent implements AfterViewInit   {  
  ...
  {    
    ...
    this.wineDeletedSubscription = wineService.wineDeletedAnnounced$.subscribe(data => {
      var items = this.wineTree.getItems();
      var itemToExpandAfterDelete = this.findPrevItem(items, this.rightClickWineId);
  ...
  wineTreeOnInitialized(): void {
    ...
    document.addEventListener('contextmenu', event => {
      ...
      if ((<Element>event.target).parentElement.className === 'jqx-tree-item-li') {
  ...
  treeListGetAll(vinId : any, getVin: boolean) {
    this.wineService.getAllWines().subscribe((res: any) => {
      ...
      if (this.selectedItem === undefined) {
        var item = this.findItemToSelect(items, vinId);
        this.wineTree.selectItem(item);
        if (getVin)
          this.getVin(item); // hent vin efter den er opdateret
      }
    });
  }
  expandCountryItem(wineTreeItems: any, wineId: any) {
    for (var i = 0; i < wineTreeItems.length; i++) {
      var treeItem = wineTreeItems[i];
  ...
  onTreeSelect(event: any): void {
    ...
    var item: any = this.wineTree.getItem(event.args.element);
    if (isNaN(item.id)) {
      if (item.isExpanded)
 ...
 getVin(treeVinItem): any {
  ...
  getWinesCount(treeListData): any {
    ...
    {
      var land = treeListData[i];
      if (land.items !== undefined)
      {
        for (var j = 0; j < land.items.length; j++) {
  ...
  getWineCount(wineTypeNode): any {   
    for (var i = 0; i < wineTypeNode.items.length; i++) {
      var vin = wineTypeNode.items[i];
